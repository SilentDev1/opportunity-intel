...
from opportunity_intel.reporting import export_vendor_ready
...
def test_contact_provenance_status_and_idempotency(db):
    ...
    enrichment = refresh_enrichment(db, opportunity, "KNOWN", "CHAIN_FRANCHISE")
...
def test_vendor_ready_export_requires_contact_and_readiness(db, tmp_path):
    ...
    refresh_enrichment(db, opportunity, "KNOWN", "INDEPENDENT_LOCAL")
    db.commit()
